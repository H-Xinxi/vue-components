...
const selectedSession = defineModel<Session | null>("selectedSession");
...
function handleClickNewSession() {
  selectedSession.value = props.sessionList[0];
}
...
function handleCommand(session: Session, command: "rename" | "delete") {
  switch (command) {
    case "rename": {
    ...
    <div class="flex-1">
      <ElScrollbar>
        <div class="flex flex-col">
          <template v-for="session of props.sessionList">
            <div
              v-if="session.messageList.length !== 0"
              class="group relative hover:bg-blue-50 rounded-xl flex h-9 px-2 items-center"
              :class="[
                {
                  'bg-blue-100!': session.id === selectedSession?.id,
